...
function RecordingRow({
  ...
  return (
    <div className="flex items-center justify-between self-stretch gap-4 p-6 relative">
      ...
      <div className="flex items-start gap-2 absolute top-1/2 right-4 -translate-y-1/2">
        <button
          disabled={isLoading}
          onClick={handlePlay}
          className="flex overflow-clip items-center py-3.25 rounded-lg justify-center shrink-0 [box-shadow:#FFFFFF_0px_0px_3px_1px_inset,#0000001A_0px_2px_3px] bg-[#FAFAFA] border border-solid border-[#0000001A] size-10 disabled:cursor-default"
        >
          {isLoading ? <Spinner /> : <PlayIcon />}
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); onDelete(); }}
          className="flex overflow-clip items-center py-3.25 rounded-lg justify-center shrink-0 [box-shadow:#FFFFFF_0px_0px_3px_1px_inset,#0000001A_0px_2px_3px] bg-[#FAFAFA] border border-solid border-[#0000001A] size-10"
        >
          <DeleteIcon />
        </button>
      </div>
      ...
      <div className="flex flex-col items-start gap-5 w-full max-w-2xl">
        ...
        <div className="flex flex-col items-start rounded-2xl self-stretch overflow-clip [box-shadow:#0000001A_0px_2px_3px] bg-white border border-solid border-[#0000001A]">
          <button
            ...
            className="flex overflow-clip items-center gap-2 px-5.5 py-3.25 justify-center self-stretch [box-shadow:#FFFFFF_0px_0px_3px_1px_inset] bg-[#FAFAFA] border-b border-b-solid border-b-[#0000001A] disabled:opacity-60 disabled:cursor-not-allowed hover:bg-[#F5F5F5] transition-colors"
          >
            {isUploading ? (
              <Spinner />
            ) : (
