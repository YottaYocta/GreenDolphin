    ...
    <AppDialog title="New Recording" open={open} onOpenChange={onOpenChange}>
      <div className="flex flex-col gap-4">
        <button
          ...
          className="btn-surface rounded-lg gap-2 h-12 w-full cursor-pointer"
        >
          <MusicNotesPlusIcon
            size={20}
            weight="fill"
        ...
        <div className="flex flex-col gap-2">
          <input
            ...
            className="w-full rounded-lg bg-surface-input h-12 px-3 font-space-mono text-sm/5 text-black/60 outline-none placeholder:text-black/25"
          />
          {error && (
          ...
          <button
            onClick={handleAdd}
            disabled={isAdding}
            className="btn-surface rounded-lg gap-2 h-12 w-full cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isAdding ? (
              <SpinnerIcon
                size={20}
